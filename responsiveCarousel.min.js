...
!0===b.lazyload&&this._lazyLoad();if(d.isFunction(b.onRedraw))b.onRedraw(c,this.internal,b)},_lazyLoad:function(){function b(a){d(a).trigger("appear")}function c(){var a;for(k=g.currentSlide;k<m;k+=1)a=l.eq(k),a.length&&a.find('img.lazy-slider:not([loaded="true"])').map(b)}function a(){d(j.target).find('img.lazy-slider:not([loaded="true"])').each(function(){d(this).trigger("appear")})}function e(){var b=d(j.target).offset().top,c=d(f).scrollTop()+d(f).height()+600,e=!1;b<c&&(a(),e=!0);return e}var g=
this.internal,h=this.instanceId,j=this.options,l=d(j.unitElement),k,m=g.currentSlide+2*g.numVisibleUnits;if(!0===d.browser.msie&&"8."===d.browser.version.substring(0,2))a();else if(!1===g.lazyloaded){c();d(f).on("load",function(){c()});if(!0===j.dragEvents){if(!1===e())d(f).on("scroll.tempevent"+h,function(){!0===e()&&d(f).unbind("scroll.tempevent"+h)})}else d.each([d(j.target),d(j.arrowLeft),d(j.arrowRight)],function(){var b=this;b.on("mouseover.tempevent",function(){a();b.unbind(".tempevent")})});
g.lazyloaded=!0}},redraw:function(){var b=this.internal,c=this.options,a=d(this.element);if(d(c.target).is(":visible")||d(c.target).not(":hidden")){void 0===this.unitWidth&&this._setUnitWidth();this._setTargetWidth();this._setArrowVisibility();if(d.isFunction(this.options.onRedraw))this.options.onRedraw(a,b,c);!0===c.lazyload&&this._lazyLoad()}},getCurrentSlide:function(){return this.internal.currentSlide},goToSlide:function(b,c){var a=this,e=this.internal,g=this.options,h=d(g.target),f;c=void 0!==
...
d(this.options.arrowRight).unbind(this.instanceId);b.removeClass("instance-"+p);b.find("img").unbind(this.instanceId);null!==this.internal.touchObject&&this.internal.touchObject.destroy();c.css({"-webkit-user-select":"","-webkit-user-drag":"","-webkit-tap-highlight-color":""});b.replaceWith(this.internal.backup)},computeAdjust:function(b){var c=this.internal,a=this.options,e=c.scrollStart.left;b=b.position().left;var g=c.maskWidth,f=0,j=c.nudgeDirection,l=c.unitWidth,k=!1;null!==j&&("left"===j&&(f=
e-l),"right"===j&&(f=e+l),"abort"===j&&(f=b),b=f,c.nudgeDirection=null);e=b+c.targetWidth;e<g&&(b=f=b+g-e,k=!0);0<b&&(b=f=0);if("individual"===a.unitWidth)for(j=0;j<c.numUnits;j+=1)if(e=d(a.unitElement).eq(j),!0===k)c.targetWidth>g&&(f=c.maskWidth-c.targetWidth);else{if(Math.abs(b)>=e.data("responsiveCarousel").left&&Math.abs(b)<=e.data("responsiveCarousel").right){f=-1*e.data("responsiveCarousel").left;break}}else f=this._round(b);!0===isNaN(f)&&(f=0);return f},getNumVisibleUnits:function(){return this.internal.numVisibleUnits}})})(jQuery,
window,document);
